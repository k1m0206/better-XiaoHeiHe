...
```text
插件需要访问 https://www.xiaoheihe.cn/app/bbs 及其子路径，以便只在小黑盒社区页面运行内容脚本。内容脚本用于调整页面布局、折叠导航菜单、隐藏右侧推荐栏，并在信息流帖子右侧展示评论预览。插件还需要访问用户在设置弹框中填写的 AI Base URL，用于用户主动开启后的模型拉取、连通测试和帖子总结请求。
```
...
不出售用户数据。
不将用户数据用于广告或追踪。
AI 入口默认开启显示；用户可选择 OpenAI Compatible、OpenAI Responses、Anthropic 或 Gemini，并可拉取模型列表或手动填写模型。用户配置 AI 服务并点击总结后，当前帖子文本和最多 30 条评论文本会由扩展后台发送到用户配置的 AI 接口用于生成总结；评论超过 30 条时优先选取点赞量更高的评论。用户关闭 AI 功能后，页面不再显示 AI 按钮。
```
