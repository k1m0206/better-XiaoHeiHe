...
插件申请 `storage` 权限，用于在用户浏览器本地保存 AI 设置、屏蔽关键词、等级过滤规则和相关本地偏好。插件申请 `declarativeNetRequestWithHostAccess` 权限，仅用于右侧评论区读取接口发送前替换请求头 Cookie，去除 `heybox_id` 和 `user_heybox_id`；插件不申请 `cookies` 权限，也不会移除或修改小黑盒 Cookie。插件申请主机权限以支持用户自定义 AI Base URL 的连通测试和总结请求。
