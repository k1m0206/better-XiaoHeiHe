...
本插件使用浏览器本地存储保存用户主动设置的偏好，包括是否屏蔽 CY 评论、评论/帖子屏蔽关键词列表、等级过滤规则，以及每个屏蔽关键词在本地生效的次数。屏蔽关键词和等级过滤规则仅用于当前浏览器内的内容过滤，不会发送到小黑盒或其他服务器。

AI 设置使用浏览器扩展存储保存在本地，包括是否开启 AI、Base URL、模型和 API Key。API Key 仅在用户测试连通或点击 AI 总结时，由扩展后台随请求发送到用户配置的 AI 接口。
