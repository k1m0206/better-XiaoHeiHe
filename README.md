...
- 新增 AI Bot 自动回复功能，支持首次风险授权确认、独立 AI 配置、模型拉取与本地模型缓存复用、回复 @、回复评论和评论首页推荐帖开关、白名单、最小 1 分钟轮询、默认只处理最近 5 分钟消息、可配置同帖同人最多回复次数（默认 5 次）、默认提供同一用户最近 20 组跨帖子历史对话、全局 30 秒评论提交尝试冷却、连续发送失败 3 次自动停用、已回复记录、首页推荐帖本地永久去重、7 天运行日志和 AI 已发回复消息日志。
